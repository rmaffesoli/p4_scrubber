...
class MockP4(object):
    def __init__(self):
        ...
        self.run_args = ()
        self.run_called = 0
        self.save_protect_called = 0
        self.save_protect_args = ()

    def save_protect(self, *args):
        self.save_protect_called = 1
        self.save_protect_args = args
        return [True]
    ...
    [
        (
            ...
            1,
            {
                ...
                'shelves': ['1', '2', '3'], 
                'groups': ['test_group'], 
                'permissions': [{'name':'depot_permission'}, {'name':'stream_permission'}, {'name':'test_permission'}]
            }
        ),
        (
            {
...
def test_run_scrubber(mocker, initial_manifest, dryrun, expected_result):
    ...
    m_get_protections_table = mocker.patch("p4_scrubber.kernel.scrubber.get_protections_table", return_value=[{'name':'test_permission'},{'name':'depot_permission'},{'name':'stream_permission'}])
    m_validate_permission = mocker.patch("p4_scrubber.kernel.scrubber.validate_permission", return_value=True)
    m_find_permissions_by_depot = mocker.patch("p4_scrubber.kernel.scrubber.find_permissions_by_depot", return_value=[{'name':'depot_permission'}])
    m_find_permissions_by_stream = mocker.patch("p4_scrubber.kernel.scrubber.find_permissions_by_stream", return_value=[{'name':'stream_permission'}])
    ...
    m_save_protections_table = mocker.patch("p4_scrubber.kernel.scrubber.save_protections_table")
    m_delete_permission = mocker.patch("p4_scrubber.kernel.scrubber.delete_permission", return_value=[{'name':'test_permission'},{'name':'depot_permission'},{'name':'stream_permission'}])
    ...
    find_permissions_by_stream_calls = [
        mocker.call(m_get_protections_table.return_value, 'test_stream'),
        mocker.call(m_get_protections_table.return_value, 'depot_stream'),
    ]
    ...
    m_validate_permission.assert_called_with(m_get_protections_table.return_value, {'name':'test_permission'})
    m_find_permissions_by_depot.assert_called_once_with(m_get_protections_table.return_value, 'test_depot')
    ...
    m_delete_permission.assert_has_calls([
        mocker.call([{'name': 'test_permission'}, {'name': 'depot_permission'}, {'name': 'stream_permission'}], {'name':'test_permission'}),
        mocker.call([{'name': 'test_permission'}, {'name': 'depot_permission'}, {'name': 'stream_permission'}], {'name':'depot_permission'}),
        mocker.call([{'name': 'test_permission'}, {'name': 'depot_permission'}, {'name': 'stream_permission'}], {'name':'stream_permission'})
    ], any_order=True)
    m_save_protections_table.assert_called_once_with([{'name':'test_permission'},{'name':'depot_permission'},{'name':'stream_permission'}], m_server, dryrun)